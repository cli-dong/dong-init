...
module.exports = function(options) {
  ...
  if (!options.force && fs.readdirSync(process.cwd()).length > 0) {
    return log.error('init', '请在非空目录上执行初始化命令！')
  }
  ...
  inquirer.prompt([{
    ...
    validate: function(input) {
      ...
      if (!/^[a-z][a-z0-9\-\.]*$/.test(input)) {
        log.warn('init', 'Must be only lowercase letters, numbers, dashes or dots, and start with lowercase letter.')
        return
      }
    ...
    validate: function(input) {
      ...
      if (!semver.valid(input)) {
        log.warn('init', 'Must be a valid semantic version (semver.org).')
        return
      }
